...
DirectX11Weaver::DirectX11Weaver(DX11WeaverInitialize dx11_weaver_initialize) {
    ...
    //this->event_stream_reader = dx11_weaver_initialize.game_bridge->GetEventManager().GetEventStream(GameBridgeEventManagerType::GB_EVENT_MANAGER_TYPE_WEAVER);

    // Get the event stream writer.
    //dx11_weaver_initialize.game_bridge->GetEventManager().CreateEventStream(GameBridgeEventManagerType::GB_EVENT_MANAGER_TYPE_WEAVER, &event_stream_writer);
}

GameBridgeManagerType DirectX11Weaver::GetEventManagerType() {
    return GameBridgeManagerType::GB_MANAGER_WEAVER_DX11;
}
