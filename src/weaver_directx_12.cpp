...
DirectX12Weaver::DirectX12Weaver(DX12WeaverInitialize dx12_weaver_initialize) {
    ...
    //this->event_stream_reader = dx12_weaver_initialize.game_bridge->GetEventManager().GetEventStream(GameBridgeEventManagerType::GB_EVENT_MANAGER_TYPE_WEAVER);

    // Get the event stream writer.
    //dx12_weaver_initialize.game_bridge->GetEventManager().CreateEventStream(GameBridgeEventManagerType::GB_EVENT_MANAGER_TYPE_WEAVER, &event_stream_writer);
}
